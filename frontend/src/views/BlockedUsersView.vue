        ...
        <table class="w-full text-left border-collapse">
          ...
          <tbody>
            ...
            <tr v-if="filtered.length === 0">
              <td colspan="5" class="py-6 text-center text-gray-500">No blocked users found.</td>
            </tr>
          </tbody>
        </table>
  ...
  if (isAdmin.value && token) {
    try {
      ...
      if (res.ok) {
        ...
        remote.forEach((r:any, i:number) => {
          const key = (r.email || '').toLowerCase() || `remote-${r.id || i}`;
          mergedByEmail[key] = { ...r };
        });

        localUsers.forEach((l:any, i:number) => {
          const key = (l.email || '').toLowerCase();
          if (key) {
            mergedByEmail[key] = { ...(mergedByEmail[key] || {}), ...l };
          } else {
            const localKey = `local-${l._localId || i}`;
            mergedByEmail[localKey] = { ...(mergedByEmail[localKey] || {}), ...l };
          }
        });
...
const filtered = computed(() => {
  ...
  return raw.value.filter(u => {
    ...
    if (!(u.blocked || u.status === 'blocked')) return false;
    if (!q) return true;
...
function persist(){
  ...
  try { window.dispatchEvent(new CustomEvent('users-updated', { detail: { count: serialized.length } })); } catch (e) { /* ignore */ }
}
