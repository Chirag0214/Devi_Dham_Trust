<template>
      ...
      <form class="mt-8 space-y-6" @submit.prevent="signup">
        <div class="space-y-4">
           <div class="relative">
            ...
            <UserIcon class="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 h-4 w-4" />
            <input v-model="signupForm.name" id="signup-name" name="name" type="text" autocomplete="name" required class="w-full pl-10 pr-3 py-3 border border-gray-300 rounded-md placeholder-gray-500 text-gray-900 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500 sm:text-sm" placeholder="Full Name" />
          </div>
          <div class="relative">
            <label for="signup-email-address" class="sr-only">Email address</label>
            <EnvelopeIcon class="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 h-4 w-4" />
            <input v-model="signupForm.email" id="signup-email-address" name="email" type="email" autocomplete="email" required class="w-full pl-10 pr-3 py-3 border border-gray-300 rounded-md placeholder-gray-500 text-gray-900 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500 sm:text-sm" placeholder="Email address" />
          </div>
          <div class="relative">
            <label for="signup-password" class="sr-only">Password</label>
            <LockClosedIcon class="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 h-4 w-4" />
            <input v-model="signupForm.password" id="signup-password" name="password" :type="showPassword ? 'text' : 'password'" autocomplete="new-password" required class="w-full pl-10 pr-3 py-3 border border-gray-300 rounded-md placeholder-gray-500 text-gray-900 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500 sm:text-sm" placeholder="Password" />
          </div>
           <div class="relative">
            <label for="confirm-password" class="sr-only">Confirm Password</label>
            <LockClosedIcon class="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 h-4 w-4" />
            <input v-model="signupForm.confirmPassword" id="confirm-password" name="confirm-password" :type="showPassword ? 'text' : 'password'" autocomplete="new-password" required class="w-full pl-10 pr-3 py-3 border border-gray-300 rounded-md placeholder-gray-500 text-gray-900 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500 sm:text-sm" placeholder="Confirm Password" />
          </div>
          <div class="flex items-center mt-2">
            <input id="show-password-signup" type="checkbox" v-model="showPassword" class="h-4 w-4 text-brand-500 focus:ring-brand-500 border-gray-300 rounded" />
  ...
  <router-link to="/login" class="font-medium text-brand-500 hover:text-brand-400 focus:outline-none">
          Sign in
        </router-link>
      </p>
    </div>
...
import { ref } from 'vue';
import { UserCircleIcon, UserIcon, EnvelopeIcon, LockClosedIcon } from '@heroicons/vue/24/solid';
